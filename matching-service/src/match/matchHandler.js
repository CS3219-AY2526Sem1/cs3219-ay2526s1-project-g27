...
const finalizeMatch = async(matchId, data, matchingQueue, topic, difficulty) => {
    ...
    if (SSEClientAConnection) {
        SSEClientAConnection.send("matchSuccess", { message: "Redirecting to collaboration space...", ...data, signedData, question: question });
        handleDisconnect(data.userA, matchingQueue);
        SSEClientAConnection.close();
    }
    if (SSEClientBConnection) {
        SSEClientBConnection.send("matchSuccess", { message: "Redirecting to collaboration space...", ...data, signedData, question: question });
