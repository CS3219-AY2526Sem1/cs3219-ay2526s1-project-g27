...
export function MatchingProvider({ children }: { children: React.ReactNode }) {
  ...
  const startMatching = () => {
    ...
    console.log(
      "🔑 JWT parts:",
      jwt?.split(".").map((part) => part.length)
    );
    ...
    matchingEventSource.onopen = () => {
      ...
      apiClient
        ...
        .catch((error) => {
          handleErrorDisplay(
            error.response?.data?.error || "Failed to connect to queue"
          );
        });
    };
  ...
  const acceptMatch = () => {
    ...
    apiClient
      .post(`/matching/matches`, matchData)
      .then((response) => {
      ...
      .catch((error) => {
        handleErrorDisplay(
          error.response?.data?.error ||
            "Something went wrong, please queue again."
        );
      });
  };
